...
**(iv)Rename some text data **
```{r}
CC$location <- gsub("'", '', CC$location)
CC$location[CC$location=="Katerina’s Café"]<-"Katerina Cafe"
```
...
So we can conclude that the most popular location for GASTech employees are Brewve Been Served and Katerina Cafe.
...
* Between 3 p.m. and 6 p.m., which is the working time, Albert's Fine Clothing appeared five times in the
  ...
  go shopping.
* Between 12pm and 2pm, a total of 55 credit card purchases were made at Ouzeri Elian, which is likely to be
  a pub, so there is speculation that some staff may have been involved in drinking on duty.
...
### 3.2 Visualization
**(i) Visualize the route for each ID **
...
Then add the gps data into the map.
...
tm_shape(bgmap) +
  ...
  tm_facets(by = "id", ncol = 1) +
  tm_layout(legend.show=FALSE) +
  tm_dots(col = "darkred",size = 0.9)
```

```{r}
p6 <- collapsibleTree(CarTrack,
