...
tm_shape(bgmap) +
  ...
  tm_facets(by = "id",ncol = 1) +
  tm_layout(legend.show=FALSE) +
  tm_dots(col = "weekday", size = 1.7)

```

Finally, we can retrieve the previous credit card data according to this location and identify which card belongs to which employee. Visualize the original credit card dataset by using DT data table.

```{r}
CC_6 <- CC %>%
  select(timestamp,last4ccnum,location)
DT::datatable(CC_6) %>% 
  formatStyle(0, 
              target = 'row', 
              lineHeight='25%')
